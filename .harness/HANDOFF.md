...
## 2026-09-20: 데모 계정 서재 전수 보강 및 토론 기억 4종 풀세트 적재
- **16권 전 도서 고화질 표지 보장 (ISBN 교정)**:
  - 교보문고 기본 대체 이미지(34,150 bytes)가 노출되던 《달과 6펜스》(`9788937493386` 세트번호 ➔ 민음사 단행본 공식 `9788937462566`)와 《지대넓얕 1》(`9791190313254` ➔ 웨일북 단행본 공식 `9791190313193`)의 ISBN을 고화질 표지 정식 번호로 교정.
  - 교보문고 CDN HTTP HEAD 검증 결과 16권 전 권이 정상 고화질 표지(`bytes != 34150`)로 100% 제공됨을 확인.
- **16권 전 도서 문장 스크랩 & 감상평 완비**:
  - 기존에 비워두었던 상위 5권(《파견자들》, 《불편한 편의점》, 《도둑맞은 집중력》, 《클린 코드》, 《그랬다고 적었다》)을 포함하여 16권 전체에 도서별 핵심 줄거리 및 주제에 맞는 인상 깊은 문장 스크랩(Scrap & RecordScrap 18건)과 감상평(Record 18건)을 탑재.
- **독서 타이머 세션 로그 36건으로 확대 및 16권 전 도서 분산**:
  - `record.reading_sessions`를 36건으로 확대 적재하여 16권 모든 책에 고른 독서 시간/페이지/날씨/시간대(새벽, 낮, 밤) 세션 로그 분포 완성 (월간 리포트 01~05번 통계 완성도 극대화).
- **토론 모드 페르소나 4종 풀세트 통찰(`agent.debate_insights`) 적재**:
  - 논리 비평가(`debate_critic`: 《클린 코드》, 《사피엔스》), 사유 상담가(`debate_counselor`: 《마흔에 읽는 쇼펜하우어》), 서사 탐험가(`debate_storyteller`: 《파견자들》), 통찰 관찰자(`debate_observer`: 《도둑맞은 집중력》) 4종 페르소나 총 5건의 실제 토론 요약 및 주제를 768차원 벡터로 생성하여 `agent.debate_insights`에 영속화.
  - 월간 리포트의 03번 핵심 토론 키워드 최우선 추출 및 AI 에이전트 대화 시 개인화 회상 메모리 제공 기반 마련.
- **품질 및 정합성 검증**:
  - DB 실환경 적재 검증: 도서 16권, 코어 스크랩 18건, 감상평 18건, 독서 세션 36건, 토론 기억 5건 전수 일치 확인.
  - `ruff check .` 및 104개 전체 단위/통합 테스트 100% Pass (3.12s) 무결점 검증 완료.

## 2026-09-21: Render Deploy Hook 자동 배포 워크플로우 연동
- **배경 및 원인**:
  - GitHub Organization(`DPYB`)의 권한 및 웹훅 동기화 누락으로 인해 `develop` 브랜치 머지 시 Render 자동 배포가 간헐적으로 트리거되지 않던 문제 해결.
- **워크플로우 구축 (`.github/workflows/deploy.yml`)**:
  - `develop` 브랜치 `push` 이벤트 및 `workflow_dispatch`(수동 재배포) 트리거 지원.
  - `concurrency` 그룹 설정을 통해 연속 머지 시 이전 배포 작업 취소 및 중복 방지.
  - 레포지토리 시크릿 `RENDER_DEPLOY_HOOK_URL` 유효성 검사 및 `curl -X POST` 응답 상태 코드(2xx) 안전 검증 로직 구현.
- **다음 단계**:
...
## 2026-09-21: [Phase 40 완료] KDC 다계층 파서 리팩토링 & 5자리 부가기호/라벨 청구기호 자동 추출 지원
- **진행한 작업**:
  - `app/core/kdc_mapper.py`:
    - `extract_kdc_code`를 단일 3자리 정규식에서 **4계층 우선순위 파서 파이프라인**으로 전면 리팩토링:
      1. 도서관 청구기호 라벨 및 접두사(`KDC 813.6-박24ㄱ`, `320.1-이38ㅅ-v.1`, `DDC 005.133-C12`, `분류기호 813.6/005` 등)에서 순수 분류기호 자동 캡처
      2. 한국 출판유통 5자리 ISBN 부가기호(예: `03320`, `93810`, `03005`, `[03810]`, `부가기호: 03320` 등)에서 독자·형태 2자리를 제외한 뒤 3자리 KDC 코드 추출
      3. 표준 3자리 정수 + 선택적 소수점(`813.6`, `843` 등)
      4. 접두사/약식 1~2자리 분류기호(`81`, `00` 등, 권차/판차 수식어 제외)
  - `tests/test_kdc_mapper.py`:
    - `kdc_to_genre` 및 `kdc_to_subject`에 청구기호 라벨 결합형, 5자리 부가기호(`03320` -> 사회과학/경제·경영, `93810` -> 문학/한국문학, `03005` -> 기술과학/컴퓨터IT) 회귀 테스트 전수 추가.
- **품질 검증**:
