...
def test_production_jwt_secret_validation():
    ...
    with pytest.raises(
        ValueError, match="CRITICAL SECURITY: JWT_SECRET_KEY must be configured"
    ):
        Settings(
            ENV="production",
...
def test_remote_database_detection():
    ...
    cfg_remote = Settings(
        DATABASE_URL="postgresql+asyncpg://user:pass@aws-0-ap-northeast-2.pooler.supabase.com:6543/postgres"
    )
