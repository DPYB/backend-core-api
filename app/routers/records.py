...
async def list_records(
    ...
    return await record_service.get_records(db, member_id, skip=skip, limit=limit)
...
async def get_record(
    ...
    """독서 기록 단건 상세 조회."""
    record = await record_service.get_record_by_id(db, member_id, record_id)
    if not record:
        raise HTTPException(
...
async def delete_record(
    ...
    success = await record_service.delete_record(db, member_id, record_id)
    if not success:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="독서 기록을 찾을 수 없습니다.",
